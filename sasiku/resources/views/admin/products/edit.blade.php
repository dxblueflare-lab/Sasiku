@extends('layouts.admin', ['title' => 'Edit Produk | SASIKU Admin', 'header' => 'Edit Produk'])

@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select
                        ...
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-violet-500 focus:ring-4 focus:ring-violet-100 transition-all outline-none"
                    >
                        <option value="kg" {{ old('unit', $product->unit ?? 'kg') === 'kg' ? 'selected' : '' }}>Kilogram (kg)</option>
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-violet-400 transition-colors relative overflow-hidden">
                    ...
                    <label for="image" class="cursor-pointer">
                        <div id="imagePreview" class="@if (!$product->image_url) hidden @endif mb-4">
                            @if ($product->image_url)
                                <img src="{{ $product->image_url }}" alt="Current image" class="max-h-48 mx-auto rounded-lg object-cover w-full aspect-square">
                            @endif
                        </div>
                        <div id="uploadPlaceholder" @if ($product->image_url) class="hidden" @endif>
                            <i data-lucide="upload-cloud" class="w-12 h-12 text-gray-400 mx-auto mb-3"></i>
                            <p class="text-gray-600 font-medium">Klik untuk ganti gambar</p>
                ...
                @enderror
                @if ($product->image_url)
                    <p class="mt-2 text-sm text-gray-500">Gambar saat ini: {{ basename($product->image_url) }}</p>
                @endif
            </div>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Bahan / Bumbu</label>
                <div class="border border-gray-200 rounded-xl p-4 max-h-60 overflow-y-auto">
                    @if($ingredients->count() > 0)
                        @foreach($ingredients as $ingredient)
                            <div class="flex items-start space-x-3 mb-3 p-2 hover:bg-gray-50 rounded-lg">
                                <input 
                                    type="checkbox" 
                                    name="ingredients[]" 
                                    value="{{ $ingredient->id }}" 
                                    ...
                                    onchange="toggleIngredientFields({{ $ingredient->id }})"
                                    {{ $product->ingredients->contains($ingredient->id) ? 'checked' : '' }}
                                >
                                <div class="flex-1">
                                    <label for="ingredient_{{ $ingredient->id }}" class="font-medium text-gray-700">{{ $ingredient->name }}</label>
                                    <p class="text-xs text-gray-500 mt-1">{{ $ingredient->description }}</p>
                                    
                                    <div id="ingredient_fields_{{ $ingredient->id }}" class="mt-2 space-y-2 {{ $product->ingredients->contains($ingredient->id) ? '' : 'hidden' }}">
                                        <div>
                                            <label class="text-xs text-gray-600">Jumlah</label>
                                            <input 
                                                type="text" 
                                                name="ingredient_quantities[{{ $ingredient->id }}]" 
                                                placeholder="Contoh: 1 kg, 2 pcs"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:border-violet-500 focus:ring-2 focus:ring-violet-100"
                                                value="{{ old('ingredient_quantities.'.$ingredient->id, $product->ingredients->find($ingredient->id)?->pivot->quantity ?? '') }}"
                                            >
                                        </div>
                                        <div>
                                            <label class="text-xs text-gray-600">Catatan</label>
                                            <textarea 
                                                name="ingredient_notes[{{ $ingredient->id }}]" 
                                                rows="2"
                                                placeholder="Catatan tambahan tentang bahan ini"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:border-violet-500 focus:ring-2 focus:ring-violet-100"
                                            >{{ old('ingredient_notes.'.$ingredient->id, $product->ingredients->find($ingredient->id)?->pivot->notes ?? '') }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <p class="text-gray-500 text-sm">Tidak ada bahan tersedia. Silakan tambahkan bahan terlebih dahulu.</p>
                    @endif
                </div>
            </div>
            ...
            <div>
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-5 h-5 text-violet-600 rounded focus:ring-violet-500">
